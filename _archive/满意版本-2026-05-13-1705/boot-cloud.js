    ...
    async function startMainApp(session) {
        ...
        pollTimer = setInterval(async () => {
            ...
            try {
                ...
                if (changed) {
                    window.__budgetApp.updateUI();
                    window.__budgetApp.updateStatsUI();
                    if (document.getElementById('income-list-page').style.display !== 'none') {
                        window.__budgetApp.renderFullIncomeList();
                    }
                    if (document.getElementById('extra-categories-page').style.display !== 'none') {
                        window.__budgetApp.updateExtraCategoriesPage();
                    }
                }
            } catch (e) {
                console.warn('sync poll', e);
            }
        }, 40000);
        ...
        document.addEventListener('visibilitychange', async () => {
            ...
            try {
                ...
                console.warn('sync visibility', e);
            }
        });
    }
