...
class DataStore {
    ...
    load() {
        ...
        } catch (e) { Object.assign(this, defaults); this.categories = DEFAULT_CATEGORIES; }
    }
    save() {
        localStorage.setItem('familyBudget_v2', JSON.stringify({
            settings: this.settings, incomes: this.incomes, expenses: this.expenses, categories: this.categories
        }));
    }
    ...
    getYearSummary(year) {
        ...
        const totalDaily = totalMonths * 6300;
...
class App {
    ...
    updateUI() {
        ...
        if (totalIncome > 0) {
            const wifeAmount = Math.max(0, totalIncome - 6300 - lastExtraTotal);
